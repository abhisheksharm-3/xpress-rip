<script lang="ts">
  ...
  function handleSubmit({ formData, formElement }: { formData: FormData; formElement: HTMLFormElement }) {
    isLoading = true;
    error = "";
...
<div class="min-h-screen bg-gradient-to-br from-red-500/5 via-background to-background">
  <div class="max-w-3xl mx-auto p-6 pt-12 space-y-8">
    ...
    <!-- Updated Search Form with form action -->
    <div in:fly={{ y: 20, duration: 700, delay: 200 }}>
      <Card class="backdrop-blur-xl bg-card/40">
        <CardContent class="pt-6">
          ...
          {#if error}
            <div
              class="mt-4 p-3 rounded-lg bg-destructive/15 text-destructive text-sm"
              transition:fade
            >
              {error}
            </div>
